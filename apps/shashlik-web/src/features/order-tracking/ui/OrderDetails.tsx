...
export function OrderDetails({ order, className, actions }: Props) {
  ...
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      <div className="rounded-[var(--r-xl)] border border-line bg-surface">
        <div className="border-b border-fg/20 p-4">
        ...
        <ul className="divide-y divide-fg/20 px-4">
          {order.lines.map((line, index) => (
            <LineRow key={`${line.productId}-${line.sizeId}-${index}`} line={line} />
          ))}
        </ul>

        <div className="flex flex-col gap-1.5 border-t border-fg/20 p-4">
          ...
          <div className="mt-1 flex items-end justify-between gap-3 border-t border-fg/20 pt-3">
